...
The input to Tensile is a tuning configuration file which provides the set of \hyperref[sec:appendixB]{parameters} used to drive the tuning process for a given iteration. Before running Tensile, the user should prepare and drop the configuration (*.yaml) file in the working dir (build*). Upon running Tensile, it reads the configuration file and perfroms the steps indicated in Figure \ref{fig:TensileSteps}. Each of the steps corresponds to the directories created by the Tensile application located within the working dir:
