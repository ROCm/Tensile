...
class TensileExactParser:
    ...
    def parse(self, line):
        ...
        try:
            ...
            #print "nums=", nums
            if len(nums) == 3:
                g = GemmConfig(nums[0], nums[1], nums[2], self.globalTA,
                               self.globalTB)
            elif len(nums) == 4:
                g = GemmConfig(nums[0], nums[1], nums[3], self.globalTA,
                               self.globalTB)
                g.batchCnt = nums[2]
            return (1, g)
...
parser = argparse.ArgumentParser(
    description=
    'convert between GEMM formats (Tensile, rocblas-bench, deepbench, CSV')

parser.add_argument(
    'infile', type=argparse.FileType('r'), help="Input file to process")
