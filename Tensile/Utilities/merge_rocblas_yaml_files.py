...
def ProcessMergeLogicFile(exactFileName, origionalFileName, outputFileName):
    ...
    mergedLibraryLogic = MergeTensileLogicFiles(libraryLogic,
                                                libraryLogicExact)
...
def RunMergeTensileLogicFiles():
    ...
    for exactLogicFilePath in exactLogicFiles:
        ...
        if os.path.isfile(origionalLogicFilePath):
            ...
            try:
                ProcessMergeLogicFile(exactLogicFilePath,
                                      origionalLogicFilePath,
                                      outputLogicFilePath)
            except Exception as ex:
                print("Exception: {0}".format(ex))

        else:
            print("# file does not exist in origional directory " +
                  origionalLogicFilePath)
