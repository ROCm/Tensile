...
namespace OldTensile
{
    ...
    {
        if(!transA && !transB) return tensile_Cijk_Ailk_Bljk_SB;
        if(!transA &&  transB) return tensile_Cijk_Ailk_Bjlk_SB;
        if( transA && !transB) return tensile_Cijk_Alik_Bljk_SB;
        if( transA &&  transB) return tensile_Cijk_Alik_Bjlk_SB;
        return nullptr;
    }

    inline TensileStatus CallOldTensile(bool transA, bool transB,
                                      float * dataD,
                                      const float * dataC,
                                      const float * dataA,
                                      const float * dataB,
